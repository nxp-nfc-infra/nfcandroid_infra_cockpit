...
@echo on
set filename=/vendor/lib64/ese_spi_nxp_snxxx.so
set prefix=nfc_nci
...
::adb shell mv /vendor/lib64/ese_spi_nxp_snxxx.so /vendor/lib64/ese_spi_nxp_snxxx.so.backup

:: Run script 
adb shell killall android.hardware.secure_element_snxxx@1.2-service
adb shell killall android.hardware.secure_element-service.nxp

adb push ../binary/pnscr_nfc /system/bin/pnscr
adb push pnscr_test_spi.txt /data/nfc/

adb shell pnscr -t 1 -d nxp-nci -f /data/nfc/pnscr_test_spi.txt


:: Restore the hal lib
adb shell mv %filename%.backup %filename%
::adb shell mv /vendor/lib64/ese_spi_nxp_snxxx.so.backup /vendor/lib64/ese_spi_nxp_snxxx.so
