...
::adb shell mv /vendor/lib64/hw/nfc_nci.default.so /system/lib64/hw/nfc_nci.default.so.backup
...
adb push sn110_init_deinit.txt /data/nfc/

adb shell pnscr -t 1 -d nxp-nci -f /data/nfc/sn110_init_deinit.txt


:: Restore the hal lib
adb shell mv %filename%.backup %filename%
::adb shell mv /vendor/lib64/hw/nfc_nci.default.so.backup /system/lib64/hw/nfc_nci.default.so
