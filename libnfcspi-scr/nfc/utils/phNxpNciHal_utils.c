...
int8_t* GKI_get_time_stamp(int8_t* tbuf, int len) {
  struct timeval val;
  struct tm* ptm;
  ...
  memset((char*)tbuf, 0x00, len);

  sprintf((char*)tbuf, "%02d:%02d:%02d.%03ld  ", ptm->tm_hour, ptm->tm_min,
          ptm->tm_sec, val.tv_usec / 1000);
...
phNxpNciHal_Monitor_t* phNxpNciHal_init_monitor(void) {
  ...
  if (nxpncihal_monitor != NULL) {
    ...
    if (pthread_mutex_init(&nxpncihal_monitor->concurrency_mutex_scr, NULL) ==
        -1) {
      NXPLOG_NCIHAL_E("concurrency_mutex_scr creation returned 0x%08x", errno);
      pthread_mutex_destroy(&nxpncihal_monitor->reentrance_mutex);
    ...
    if (listInit(&nxpncihal_monitor->sem_list) != 1) {
      ...
      pthread_mutex_destroy(&nxpncihal_monitor->concurrency_mutex);
      pthread_mutex_destroy(&nxpncihal_monitor->concurrency_mutex_scr);
      pthread_mutex_destroy(&nxpncihal_monitor->reentrance_mutex);
      goto clean_and_return;
    }
  } else {
...
void phNxpNciHal_print_packet(const char* pString, const uint8_t* p_data,
  ...
  if (0 == memcmp(pString, "SEND", 0x04)) {
    ...
    printf("%s RCEIVE : %3d < %s\n", GKI_get_time_stamp(tbuf, LEN_TIME_BUF),
           len, print_buffer);
