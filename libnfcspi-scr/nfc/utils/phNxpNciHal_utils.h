...
typedef struct phNxpNciHal_Monitor {
  ...
  /* Mutex protecting native library against concurrency */
  pthread_mutex_t concurrency_mutex_scr;
...
#define CONCURRENCY_LOCK_SCR()   \
  if (phNxpNciHal_get_monitor()) \
  pthread_mutex_lock(&phNxpNciHal_get_monitor()->concurrency_mutex_scr)
#define CONCURRENCY_UNLOCK_SCR() \
  if (phNxpNciHal_get_monitor()) \
  pthread_mutex_unlock(&phNxpNciHal_get_monitor()->concurrency_mutex_scr)
