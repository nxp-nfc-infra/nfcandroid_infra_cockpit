 ...
 * reading and writing data into files provided by the driver interface.
 *
 * API listed here encompasses Transport Mapping Layer interfaces required to be
 * mapped to different Interfaces and Platforms.
 *
 */
...
/*
 * Value indicates size of time buffer
 */
#define LEN_TML_TIME_BUF 32
...
typedef struct phTmlNfc_Config {
  /* Port name connected to PN54AD
   *
   * Platform specific canonical device name to which PN548AD is connected.
   *
   * e.g. On Linux based systems this would be /dev/PN548AD
   */
  char *pDevName;
  /* Callback Thread ID
   *
  ...
  /* Communication speed between DH and PN548AD
   *
   * This is the baudrate of the bus for communication between DH and PN548AD */
  uint32_t dwBaudRate;
